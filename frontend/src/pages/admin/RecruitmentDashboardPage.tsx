...
import { Users, Calendar, CheckCircle, XCircle, Loader2, ChevronDown, ChevronUp, Send } from 'lucide-react';
    ...
    <div className="border border-gray-100 rounded-xl overflow-hidden">
      <div className="flex items-center gap-3 p-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {app.status === 'pending' && (
            <>
              <button onClick={() => onStatusChange(app.id, 'shortlisted')} disabled={updating} title="Shortlist"
                className="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center justify-center transition-colors">
                <CheckCircle size={14} />
              </button>
              <button onClick={() => onStatusChange(app.id, 'rejected')} disabled={updating} title="Reject"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <Card className="w-full max-w-md space-y-4">
        ...
        <div className="space-y-3">
          {[
            { label: 'Date & Time *', type: 'datetime-local', value: scheduledAt, onChange: setScheduledAt },
            { label: 'Location (optional)', type: 'text', value: location, onChange: setLocation, placeholder: 'Room 204, Block B…' },
            { label: 'Google Meet / Zoom link (optional)', type: 'url', value: meetLink, onChange: setMeetLink, placeholder: 'https://meet.google.com/…' },
    ...
    <div className="border border-gray-100 rounded-xl p-4 space-y-3">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500">
            {new Date(slot.scheduledAt).toLocaleString('en-US', { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })} · {slot.durationMins}min
          </p>
        </div>
        <span className={cn('px-2.5 py-0.5 rounded-full text-xs font-semibold',
          slot.result === 'pending' ? 'bg-amber-100 text-amber-700' :
          slot.result === 'accepted' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600')}>
          {slot.result}
        </span>
      </div>
      {(slot.location || slot.meetLink) && (
        <div className="flex items-center gap-3 text-xs text-gray-500">
          ...
          {slot.meetLink && <a href={slot.meetLink} target="_blank" rel="noreferrer" className="text-indigo-600 hover:underline">🔗 Join Meeting</a>}
        </div>
      )}
        ...
        <div className="space-y-2">
          {showNotes && (
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
              placeholder="Interview notes (optional)…" rows={2}
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-2 ml-auto">
              <button onClick={() => onResult(slot.id, 'rejected', notes)}
              ...
              <button onClick={() => onResult(slot.id, 'accepted', notes)}
...
const RecruitmentDashboardPage: React.FC = () => {
  ...
  // FIX: async load function — setLoading(false) runs asynchronously in
  // finally, not synchronously in the effect body.
  const load = useCallback(async () => {
    if (!clubId) return;
    setLoading(true);
    try {
      const [apps, slots] = await Promise.all([fetchApplications(clubId), fetchInterviews(clubId)]);
      ...
      <div className="flex gap-2 bg-gray-100 p-1 rounded-xl w-fit">
        {([
          ...
          <button key={key} onClick={() => setTab(key)}
            ...
            {icon} {label}
            {count > 0 && (
              <span className="px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-100 text-indigo-700">{count}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {loading ? (
          ...
          {applications.length === 0 ? (
            ...
            applications.map((app) => (
              <ApplicationRow key={app.id} app={app} onStatusChange={handleStatusChange}
                onSchedule={setScheduleTarget} updating={updating} />
            ))
          )}
        </div>
      ) : (
        <div className="space-y-3">
          {interviews.length === 0 ? (
            ...
            interviews.map((slot) => <InterviewRow key={slot.id} slot={slot} onResult={handleResult} />)
          )}
        </div>
      )}

      {scheduleTarget && clubId && (
        <ScheduleModal app={scheduleTarget} clubId={clubId}
          onClose={() => setScheduleTarget(null)} onScheduled={load} />
      )}
    </div>
  );
};
