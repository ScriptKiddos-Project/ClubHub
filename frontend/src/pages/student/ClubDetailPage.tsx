...
const MOCK_CLUB: Club = {
  ...
  category: "technology",
  description:
    "Focused on cutting-edge AI research, neural architecture exploration, and real-world machine learning applications. We host weekly paper readings, monthly project showcases, and the annual DeepLink Hackathon. Open to all students passionate about AI, regardless of prior experience.",
  memberCount: 52,
  status: "approved",
  isJoined: false,
...
const ClubDetailPage: React.FC = () => {
  ...
  const isJoined =
    joinedOverride !== null ? joinedOverride : (club.isJoined ?? false);

  const [joining, setJoining] = useState(false);
  const [activeTab, setActiveTab] = useState<"about" | "events" | "members">(
    "about",
  );
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-1.5 text-sm text-gray-500">
            ...
            {club.upcomingEventCount !== undefined && (
              <span className="flex items-center gap-1">
                <Calendar size={13} /> {club.upcomingEventCount} upcoming events
              </span>
            )}
          </div>
        </div>
            ...
            {isJoined && (
              <Card className="bg-indigo-50 border-indigo-100">
                ...
                <p className="text-xs text-indigo-500">
                  You'll receive announcements and event updates from this club.
                </p>
              </Card>
            )}
          </div>
